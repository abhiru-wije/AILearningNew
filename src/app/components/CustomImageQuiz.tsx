...
import { IItem, ImageQuizItem, IRows } from "@/types/types";
...
interface IQuizClient {
  quizDetails: ImageQuizItem;
  lessonID: string;
  data: IRows;
  onSuccess: () => void;
}

export default function CustomImageQuiz({
  quizDetails,
  lessonID,
  data,
  onSuccess,
  ...
  const handleSubmit = (
    ...
    const res = rows.Answer.items.map((item) => item.content);
    ...
    if (
      ...
      res.every((val, index) => val === answer[index])
    ) {
      setIsSuccess(true);
      ...
      <main className="fixed inset-0 z-40 flex justify-center items-center h-screen w-screen">
        ...
        >
          <div className="wrapper bg-gray-100 md:w-3/5 w-full h-4/5 rounded-xl p-10 shadow-xl">
            ...
            <div className="option-wrapper mt-16 flex items-start gap-2 h-28">
...
function Option({ item, provided }: { item: IItem; provided: any }) {
  return (
    <div
      ...
      className="flex gap-3 w-60 h-60 overflow-hidden relative justify-center items-center"
      style={{
        userSelect: "none",
    ...
    >
      <Image
        src={`/assets/images/${item.content}.jpg`}
